...
if ( !isset($_FILES["fImage_servicio"]) || $_FILES["fImage_servicio"]["error"] > 0 || !isset($_FILES["fImage_Croquis"]) || $_FILES["fImage_Croquis"]["error"] > 0){
  ...
  if (in_array($_FILES['fImage_servicio']['type'], $permitidos) && $_FILES['fImage_servicio']['size'] <= $limite_kb * 1024){
    ...
    $resultado = @mysql_query("INSERT INTO 
      Reportes (ID_Usuario, Calle, Numero, Colonia, Codigo_Postal, Calle_Uno, Calle_Dos, Descripcion, Estatus, Departamento, Imagen, Tipo_imagen, Croquis, Tipo_Croquis) 
      VALUES ('$id', '$calle', '$numero',  '$colonia', '$cp', '$calle_uno', '$calle_dos', '$descripcion', '$status', '$departamento', '$data', '$tipo', '$data_croquis', '$tipo_croquis')");
    if ($resultado){
